...
func TestOperatorRecoveryEnforcesRoleScopeAndBindingBeforeSettling(t *testing.T) {
	...
	records, err := h.evidence.ReadEvidence(context.Background(), verifierAuthority(t), testRunID, 0)
	if err != nil {
		t.Fatal(err)
	}
	found := false
...
// RecordedEvidence delegates: the wrapper refuses writes, never lookups, so a
// producer converging on a recorded fact sees exactly what the store holds.
func (r *refusingEvidence) RecordedEvidence(ctx context.Context, scope events.Scope, evidenceID string) (events.RecordedEvidence, bool, error) {
	return r.inner.RecordedEvidence(ctx, scope, evidenceID)
}
...
func TestOperatorResolutionConvergesWhenEvidenceFailsAfterTheJournalIsDecided(t *testing.T) {
	...
	after, err := h.evidence.ReadEvidence(context.Background(), verifierAuthority(t), testRunID, 0)
	if err != nil {
		t.Fatal(err)
	}
	stored := 0
