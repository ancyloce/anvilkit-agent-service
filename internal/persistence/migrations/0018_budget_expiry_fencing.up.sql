-- Clock expiry fences a reservation: it withdraws the hold's authority to
-- authorize new dispatch. It does not manufacture attempt finality and does
-- not release the hold. A clock can prove that a lifetime elapsed; it cannot
-- witness what a physical attempt spent, and treating its silence as a final
-- cost both understates real exposure and takes away headroom a still-running
-- attempt is entitled to. The fence is a column on the row so the invariant is
...
CREATE INDEX IF NOT EXISTS budget_reservations_reconcile_idx
    ON agent_control.budget_reservations(workspace_id, project_id, root_run_id)
    WHERE expired AND NOT released;
