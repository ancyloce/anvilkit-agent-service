...
func writeRunAuthority(t *testing.T, definitionID, definitionDigest string) string {
	...
	//
	// The custodian carries the custody capabilities and the artifact-content
	// clearance on its own subject record. So does the pretender, deliberately:
	// it holds every custody grant there is and is admitted under an ordinary
	// role, which is what proves the role and the grants are two independent
	// keys rather than one spelled twice.
	raw := `{"scope":{"workspaceId":"workspace","projectId":"project"},"change":{"generation":1,"authorizedBy":"operator.slice","reason":"controlled vertical slice authority","ticket":"CHG-SLICE-0001"},"subjects":[{"actorId":"actor","role":"agent-actor"},{"actorId":"reviewer","role":"agent-reviewer"},{"actorId":"operator","role":"agent-operator"},{"actorId":"impostor","role":"agent-actor"},{"actorId":"custodian","role":"agent-artifact-custodian","custodyCapabilities":["artifact-custody.legal-hold","artifact-custody.delete"],"dataClasses":["internal"]},{"actorId":"pretender","role":"agent-actor","custodyCapabilities":["artifact-custody.legal-hold","artifact-custody.delete"],"dataClasses":["internal"]}],"definition":{"definitionId":"` + definitionID + `","definitionDigest":"` + definitionDigest + `"},"contractBomReference":{"repository":"anvilkit/contracts","bomDigest":"sha256:` + strings.Repeat("a", 64) + `","ociManifestDigest":"sha256:` + strings.Repeat("b", 64) + `","evidenceManifestDigest":"sha256:` + strings.Repeat("c", 64) + `"},"policy":` + policy + `,"budget":{"kind":"AgentBudget","modelLimits":{"maximumCalls":10,"maximumConcurrentCalls":2},"tokenLimits":{"inputTokens":4096,"outputTokens":2048,"totalTokens":6144},"workerLimits":{"maximumAttempts":4,"maximumDurationMilliseconds":60000},"gpuLimits":{"maximumGpuMilliseconds":0},"currencyLimits":{"maximumCost":{"amount":"1000","currency":"USD"},"reservedCost":{"amount":"500","currency":"USD"}},"reservationId":"reservation.synthetic.001","exceedBehavior":"refuse","policy":` + policy + `}}`
